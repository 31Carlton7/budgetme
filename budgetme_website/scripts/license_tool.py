...
def line_prepender(filename: str, line):
    ...
    if filename[-3:] == ".ts" or filename[-4:] == ".css":
        with open(filename, 'r+') as f:
            ...
            f.close()
    elif filename[-7:] == ".svelte":
        with open(filename, 'r+') as f:
            content = f.read()
            if content.find(license_text_svelte) == -1:
                f.seek(0, 0)
                f.write(line.rstrip('\r\n') + '\n\n' + content)
...
for file in getListOfFiles(rootdir):
    if file[-7:] == ".svelte":
        line_prepender(file, license_text_svelte)
    else:
